...
# regexp of character
my $qq_char   = qr/\\c[\x40-\x5F]|\\?(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[\x00-\xFF])/oxms;
my  $q_char   = qr/\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[\x00-\xFF]/oxms;
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Eeuctw::function_ord     = 'Char::EUCTW::ord'     if /\A ord \z/oxms;
                    $Char::Eeuctw::function_ord_    = 'Char::EUCTW::ord_'    if /\A ord \z/oxms;
                    $Char::Eeuctw::function_reverse = 'Char::EUCTW::reverse' if /\A reverse \z/oxms;
                    $Char::Eeuctw::function_getc    = 'Char::EUCTW::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::EUCTW::escape();
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::EUCTW software provides)

Char::EUCTW software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::EUCTW software provides)

Char::EUCTW software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Eeuctw::ignorecase()' (Char::EUCTW software provides)

Char::EUCTW software applies calling 'Char::Eeuctw::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eeuctw::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::EUCTW software provides)

Char::EUCTW software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::EUCTW software provide)
...
=item * Eval EUC-TW Script
  ...
  $eval = Char::EUCTW::eval { block };
  $eval = Char::EUCTW::eval $expr;
  $eval = Char::EUCTW::eval;

  The Char::EUCTW::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::EUCTW::eval { block }
  and Char::EUCTW::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::EUCTW::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::EUCTW::eval, the value returned from an Char::EUCTW::eval is
  ...
  Char::EUCTW::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::EUCTW::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::EUCTW::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::EUCTW::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::EUCTW::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::EUCTW::eval cannot trap syntax errors.

  The Char::EUCTW::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::EUCTW::eval is complete, as do any subroutine or format definitions.
  The code of the Char::EUCTW::eval is treated as a block, so any locally scoped
  variables declared within the Char::EUCTW::eval last only until the Char::EUCTW::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::EUCTW::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::EUCTW::eval { block }    eval { block }
  Char::EUCTW::eval $expr        eval Char::EUCTW::escape $expr
  Char::EUCTW::eval              eval Char::EUCTW::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::EUCTW::eval does not exist. If it exists,
  you will troubled, when Char::EUCTW::eval has a parameter that is single quoted
  string included my variables. Char::EUCTW::escape is a subroutine that makes Perl
  ...
  Since Char::EUCTW::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::EUCTW::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::EUCTW::eval $expr and Char::EUCTW::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::EUCTW::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::EUCTW::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::EUCTW::eval '$answer =';   # sets $@
  ...
  Using the Char::EUCTW::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::EUCTW::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::EUCTW::eval { die "foo lives here" };
  ...
  With an Char::EUCTW::eval, you should be especially careful to remember what's being
  looked at when:

      Char::EUCTW::eval $x;        # CASE 1
      Char::EUCTW::eval "$x";      # CASE 2

      Char::EUCTW::eval '$x';      # CASE 3
      Char::EUCTW::eval { $x };    # CASE 4

      Char::EUCTW::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::EUCTW::eval { test_repugnancy() };
  ...
  The block of Char::EUCTW::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::EUCTW software provides)

Char::EUCTW software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::EUCTW software and
