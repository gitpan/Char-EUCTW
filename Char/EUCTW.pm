...
######################################################################
#
# Char::EUCTW - Source code filter to escape EUC-TW script
#
...
BEGIN { CORE::require Char::Eeuctw; }
...
# regexp of character
my $your_char = q{\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[\x00-\xFF]};
...
my $function_ord;         # ord()   to ord()   or Char::EUCTW::ord()
my $function_ord_;        # ord     to ord     or Char::EUCTW::ord_
my $function_reverse;     # reverse to reverse or Char::EUCTW::reverse
my $function_getc;        # getc    to getc    or Char::EUCTW::getc
...
END
    ...
    print Char::EUCTW::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Eeuctw::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::EUCTW::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Eeuctw::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::EUCTW::escape_script {
    ...
    # read EUC-TW script
    my $fh = gensym();
    Char::Eeuctw::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Eeuctw(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Eeuctw '%s.0'; # 'quote' for perl5.6\n", $Char::EUCTW::VERSION); # require run-time routines version

        # use Char::EUCTW version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::EUCTW \s* ([^\x8E\xA1-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::EUCTW::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/EUCTW.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/EUCTW.pm $version, but @{[__FILE__]} is version $Char::EUCTW::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Eeuctw.pm $version, but \\\$Char::Eeuctw::VERSION is \$Char::Eeuctw::VERSION" if \$Char::Eeuctw::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::EUCTW::VERSION) {
                    die "Script $0 required Char/EUCTW.pm $version, but @{[__FILE__]} is only version $Char::EUCTW::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::EUCTW::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::EUCTW::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::EUCTW::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::EUCTW::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::EUCTW::re_t=%s; local $Char::EUCTW::re_a=''; while($Char::EUCTW::re_t =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s$Char::EUCTW::re_t="$Char::EUCTW::re_a${1}$Char::EUCTW::re_r$'"; pos($Char::EUCTW::re_t)=length "$Char::EUCTW::re_a${1}$Char::EUCTW::re_r"; $Char::EUCTW::re_a=substr($Char::EUCTW::re_t,0,pos($Char::EUCTW::re_t)); } return $Char::EUCTW::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::EUCTW::re_r=eval $Char::EUCTW::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::EUCTW::re_t=%s; while($Char::EUCTW::re_t =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s$Char::EUCTW::re_t="$`$Char::EUCTW::re_r$'"; pos($Char::EUCTW::re_t)=length "$`$Char::EUCTW::re_r"; } return $Char::EUCTW::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::EUCTW::re_r=%s; %s"%s$Char::EUCTW::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::EUCTW::re_n=0; local $Char::EUCTW::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="$Char::EUCTW::re_a${1}$Char::EUCTW::re_r$'"; pos(%s)=length "$Char::EUCTW::re_a${1}$Char::EUCTW::re_r"; $Char::EUCTW::re_a=substr(%s,0,pos(%s)); $Char::EUCTW::re_n++} return %s$Char::EUCTW::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::EUCTW::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="$`$Char::EUCTW::re_r$'"; pos(%s)=length "$`$Char::EUCTW::re_r"; $Char::EUCTW::re_n++} return %s$Char::EUCTW::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="%s$Char::EUCTW::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="%s$Char::EUCTW::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::EUCTW::re_r=eval $Char::EUCTW::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/EUCTW.pm and Char/Eeuctw.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::EUCTW;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::EUCTW;
  use Char::EUCTW ver.sion;             --- require minimum version
  use Char::EUCTW ver.sion.0;           --- expects version (match or die)
  use Char::EUCTW qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::EUCTW ver.sion qw(ord reverse getc);
  use Char::EUCTW ver.sion.0 qw(ord reverse getc);

  # "no Char::EUCTW;" not supported

  or

  $ perl Char/EUCTW.pm EUC-TW_script.pl > Escaped_script.pl.e
...
Char::EUCTW software is "middleware" between perl interpreter and your Perl script
written in EUC-TW.
...
By "use Char::EUCTW;", it automatically interpret your script as EUC-TW. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/EUCTW.pm, Char/Eeuctw.pm
  +---------------------------------------+
...
import of Char/Eeuctw.pm works well.
...
=head1 Software Composition
   ...
   Char/EUCTW.pm               --- source code filter to escape EUC-TW
   Char/Eeuctw.pm              --- run-time routines for Char/EUCTW.pm
...
You need write 'use Char::EUCTW;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::EUCTW;
  ---------------------------------

=head1 Multiple-Octet Anchoring of Regular Expression (Char/EUCTW.pm provides)

Char/EUCTW.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Eeuctw::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Multiple-Octet Character Regular Expression (Char/EUCTW.pm provides)

Char/EUCTW.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Eeuctw::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Eeuctw::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Eeuctw::ignorecase()' (Char/EUCTW.pm provides)

Char/EUCTW.pm applies calling 'Char::Eeuctw::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Eeuctw::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Eeuctw::matched/
  m/regexp/               m/ditto$Char::Eeuctw::matched/
  ?regexp?                m?ditto$Char::Eeuctw::matched?
  m?regexp?               m?ditto$Char::Eeuctw::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Eeuctw::matched/) ?
  s/regexp/replacement/   eval{ Char::Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eeuctw::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Eeuctw::split(qr/regexp/)
  split(m/regexp/)        Char::Eeuctw::split(qr/regexp/)
  split(qr/regexp/)       Char::Eeuctw::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Eeuctw::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Eeuctw::matched/
  m/regexp/b              m/(?:regexp)$Char::Eeuctw::matched/
  ?regexp?b               m?regexp$Char::Eeuctw::matched?
  m?regexp?b              m?regexp$Char::Eeuctw::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Eeuctw::matched/) ?
  s/regexp/replacement/b  eval{ Char::Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Eeuctw::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Eeuctw.pm provides)
...
Definitions in Char/Eeuctw.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Char::Eeuctw::anchor}         qr{\G(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])*?}
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{.*?[^\x8E\xA1-\xFE](?:\x8E[\xA2-\xB0][\xA1-\xFE]{2}|[\xA1-\xFE]{2})*?}oxms;
                           for over 32766 octets string on ActivePerl5.6 and Perl5.10 or later
                           qr{\G(?(?=.{0,32766}\z)(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[$sbcs](?:$tbcs_1st[^$sbcs]{2})*?)))}oxms;
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char/EUCTW.pm provides)

Char/EUCTW.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/EUCTW.pm and Char/Eeuctw.pm provide)

Insert 'Char::Eeuctw::' at head of function name. Char/Eeuctw.pm provides your script Char::Eeuctw::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Eeuctw::split     Character
  tr///       Char::Eeuctw::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Eeuctw::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Eeuctw::chop      Character
  index       Char::Eeuctw::index     Character
  rindex      Char::Eeuctw::rindex    Character
  lc          Char::Eeuctw::lc        Character
  lcfirst     Char::Eeuctw::lcfirst   Character
  uc          Char::Eeuctw::uc        Character
  ucfirst     Char::Eeuctw::ucfirst   Character
  fc          Char::Eeuctw::fc        Character
  chr         Char::Eeuctw::chr       Character
  glob        Char::Eeuctw::glob      Character
  -------------------------------------------
...
You need write 'Char::EUCTW::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        Char::EUCTW::ord
  reverse    Char::EUCTW::reverse
  getc       Char::EUCTW::getc
  length     Char::EUCTW::length
  substr     Char::EUCTW::substr
  index      Char::EUCTW::index          See 'About Indexes'
  rindex     Char::EUCTW::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Eeuctw::index)
  Char::EUCTW::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Eeuctw::rindex)
  Char::EUCTW::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  If you import ord "use Char::EUCTW qw(ord);", ord of your script will be rewritten in
  Char::EUCTW::ord. Char::EUCTW::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::EUCTW::reverse(@list);
  $reverse = Char::EUCTW::reverse(@list);
  ...
  If you import reverse "use Char::EUCTW qw(reverse);", reverse of your script will be
  rewritten in Char::EUCTW::reverse. Char::EUCTW::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::EUCTW::getc(FILEHANDLE);
  $getc = Char::EUCTW::getc($filehandle);
  $getc = Char::EUCTW::getc;
  ...
  If you import getc "use Char::EUCTW qw(getc);", getc of your script will be rewritten
  in Char::EUCTW::getc. Char::EUCTW::getc is not compatible with getc of JPerl.

=item * Length by EUC-TW Character

  $length = Char::EUCTW::length($string);
  $length = Char::EUCTW::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the Char::EUCTW::length
  of $_.

  Do not try to use Char::EUCTW::length to find the size of an array or hash. Use scalar
...
=item * Substr by EUC-TW Character
  ...
  $substr = Char::EUCTW::substr($string,$offset,$length,$replacement);
  $substr = Char::EUCTW::substr($string,$offset,$length);
  $substr = Char::EUCTW::substr($string,$offset);
  ...
  my $end    = Char::EUCTW::substr $s, 14;        # climbed the green tree
  my $tail   = Char::EUCTW::substr $s, -4;        # tree
  my $z      = Char::EUCTW::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::EUCTW::substr() subroutine as an
  ...
  end of the string, Char::EUCTW::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::EUCTW::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::EUCTW::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::EUCTW::substr $name, 7;      # returns undef, with warning
  Char::EUCTW::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::EUCTW::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::EUCTW::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::EUCTW::substr() acts
...
=item * Index by EUC-TW Character
  ...
  $index = Char::EUCTW::index($string,$substring,$offset);
  $index = Char::EUCTW::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::EUCTW::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by EUC-TW Character
  ...
  $rindex = Char::EUCTW::rindex($string,$substring,$offset);
  $rindex = Char::EUCTW::rindex($string,$substring);

  This subroutine works just like Char::EUCTW::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::EUCTW::index).
  ...
  $pos = Char::EUCTW::length($string);
  while (($pos = Char::EUCTW::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Eeuctw::glob() subroutine. See Char::Eeuctw::glob
  of Char/Eeuctw.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::EUCTW qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::EUCTW qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Subroutines (Char/EUCTW.pm provides)

Char/EUCTW.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/Char::EUCTW and change
'use utf8;' to 'use Char::EUCTW;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Eeuctw.pm provides these
  functions.
...
=item * Char::EUCTW::substr as Lvalue

If Perl version is older than 5.14, Char::EUCTW::substr differs from CORE::substr, and
...
Char::EUCTW::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${`}            Char::Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Eeuctw::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Eeuctw::MATCH()       $1
  ${&}            Char::Eeuctw::MATCH()       $1
  $MATCH          Char::Eeuctw::MATCH()       $1
  ${^MATCH}       Char::Eeuctw::MATCH()       $1
  $'              $'                   $'
  ${'}            ${'}                 $'
  $POSTMATCH      Char::Eeuctw::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Eeuctw::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl,japerl    Encode,Char::EUCTW
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Char::EUCTW software remains one language and one interpreter.
