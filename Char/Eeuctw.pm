...
######################################################################
#
# Char::Eeuctw - Run-time routines for Char/EUCTW.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Char::Eeuctw::index($name, '::') == -1) && (Char::Eeuctw::index($name, "'") == -1)) {
            my $pkg;
...
sub import {
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        ...
        for (@ARGV) {
            ...
            # has space
            if (/\A (?:$q_char)*? [ ] /oxms) {
                if (my @glob = Char::Eeuctw::glob(qq{"$_"})) {
                    push @argv, @glob;
                }
                else {
                    push @argv, $_;
...
#
sub unimport {}
sub Char::Eeuctw::split(;$$$);
sub Char::Eeuctw::tr($$$$;$);
sub Char::Eeuctw::chop(@);
sub Char::Eeuctw::index($$;$);
sub Char::Eeuctw::rindex($$;$);
sub Char::Eeuctw::lcfirst(@);
sub Char::Eeuctw::lcfirst_();
sub Char::Eeuctw::lc(@);
sub Char::Eeuctw::lc_();
sub Char::Eeuctw::ucfirst(@);
sub Char::Eeuctw::ucfirst_();
sub Char::Eeuctw::uc(@);
sub Char::Eeuctw::uc_();
sub Char::Eeuctw::fc(@);
sub Char::Eeuctw::fc_();
sub Char::Eeuctw::ignorecase;
sub Char::Eeuctw::classic_character_class;
sub Char::Eeuctw::capture;
sub Char::Eeuctw::chr(;$);
sub Char::Eeuctw::chr_();
sub Char::Eeuctw::glob($);
sub Char::Eeuctw::glob_();

sub Char::EUCTW::ord(;$);
sub Char::EUCTW::ord_();
sub Char::EUCTW::reverse(@);
sub Char::EUCTW::getc(;*@);
sub Char::EUCTW::length(;$);
sub Char::EUCTW::substr($$;$$);
sub Char::EUCTW::index($$;$);
sub Char::EUCTW::rindex($$;$);
sub Char::EUCTW::escape(;$);
...
${Char::Eeuctw::anchor}      = qr{\G(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])*?}oxms;
...
if (($] >= 5.010001) or
    ...
    # EUC-TW encoding
        ...
        ${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[^\x8E\xA1-\xFE]     (?:            \x8E[\xA1-\xB0][\xA1-\xFE]{2}|               [\xA1-\xFE]{2})*?}oxms;
    ...
    # other encoding
        ...
        ${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:$tbcs_1st[^$sbcs]{2})*?}oxms;
    }

    ${Char::Eeuctw::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
...
#                                                                                      (?(?=[$sbcs]+\z) # (6)
#                                                                                                      .*?| #(7)
#                                                                                                          (?:${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17}) # (8)
#                                                                                                                                                           ))}oxms;

    # avoid: Complex regular subexpression recursion limit (32766) exceeded at here.
    local $^W = 0;

    if (((('A' x 32768).'B') !~ / ${Char::Eeuctw::anchor}                              B /oxms) and
        ((('A' x 32768).'B') =~ / ${Char::Eeuctw::anchor_SADAHIRO_Tomoyuki_2002_01_17} B /oxms)
    ) {
        ${Char::Eeuctw::anchor} = ${Char::Eeuctw::anchor_SADAHIRO_Tomoyuki_2002_01_17};
    }
    else {
        undef ${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17};
    }
}
...
${Char::Eeuctw::dot}         = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x0A])};
${Char::Eeuctw::dot_s}       = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])};
${Char::Eeuctw::eD}          = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE0-9])};
...
# ${Char::Eeuctw::eS}        = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x09\x0A    \x0C\x0D\x20])};
# ${Char::Eeuctw::eS}        = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x09\x0A\x0B\x0C\x0D\x20])};
${Char::Eeuctw::eS}          = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\s])};

${Char::Eeuctw::eW}          = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE0-9A-Z_a-z])};
${Char::Eeuctw::eH}          = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x09\x20])};
${Char::Eeuctw::eV}          = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x0A\x0B\x0C\x0D])};
${Char::Eeuctw::eR}          = qr{(?:\x0D\x0A|[\x0A\x0D])};
${Char::Eeuctw::eN}          = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x0A])};
${Char::Eeuctw::not_alnum}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x30-\x39\x41-\x5A\x61-\x7A])};
${Char::Eeuctw::not_alpha}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x41-\x5A\x61-\x7A])};
${Char::Eeuctw::not_ascii}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x00-\x7F])};
${Char::Eeuctw::not_blank}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x09\x20])};
${Char::Eeuctw::not_cntrl}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x00-\x1F\x7F])};
${Char::Eeuctw::not_digit}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x30-\x39])};
${Char::Eeuctw::not_graph}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x21-\x7F])};
${Char::Eeuctw::not_lower}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x61-\x7A])};
${Char::Eeuctw::not_lower_i} = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])};
${Char::Eeuctw::not_print}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x20-\x7F])};
${Char::Eeuctw::not_punct}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])};
${Char::Eeuctw::not_space}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\s\x0B])};
${Char::Eeuctw::not_upper}   = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x41-\x5A])};
${Char::Eeuctw::not_upper_i} = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])};
${Char::Eeuctw::not_word}    = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x30-\x39\x41-\x5A\x5F\x61-\x7A])};
${Char::Eeuctw::not_xdigit}  = qr{(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE\x30-\x39\x41-\x46\x61-\x66])};
${Char::Eeuctw::eb}          = qr{(?:\A(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]|\z))};
${Char::Eeuctw::eB}          = qr{(?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?<=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF])(?=[\x00-\x2F\x40\x5B-\x5E\x60\x7B-\xFF]))};

# avoid: Name "Char::Eeuctw::foo" used only once: possible typo at here.
${Char::Eeuctw::dot}         = ${Char::Eeuctw::dot};
${Char::Eeuctw::dot_s}       = ${Char::Eeuctw::dot_s};
${Char::Eeuctw::eD}          = ${Char::Eeuctw::eD};
${Char::Eeuctw::eS}          = ${Char::Eeuctw::eS};
${Char::Eeuctw::eW}          = ${Char::Eeuctw::eW};
${Char::Eeuctw::eH}          = ${Char::Eeuctw::eH};
${Char::Eeuctw::eV}          = ${Char::Eeuctw::eV};
${Char::Eeuctw::eR}          = ${Char::Eeuctw::eR};
${Char::Eeuctw::eN}          = ${Char::Eeuctw::eN};
${Char::Eeuctw::not_alnum}   = ${Char::Eeuctw::not_alnum};
${Char::Eeuctw::not_alpha}   = ${Char::Eeuctw::not_alpha};
${Char::Eeuctw::not_ascii}   = ${Char::Eeuctw::not_ascii};
${Char::Eeuctw::not_blank}   = ${Char::Eeuctw::not_blank};
${Char::Eeuctw::not_cntrl}   = ${Char::Eeuctw::not_cntrl};
${Char::Eeuctw::not_digit}   = ${Char::Eeuctw::not_digit};
${Char::Eeuctw::not_graph}   = ${Char::Eeuctw::not_graph};
${Char::Eeuctw::not_lower}   = ${Char::Eeuctw::not_lower};
${Char::Eeuctw::not_lower_i} = ${Char::Eeuctw::not_lower_i};
${Char::Eeuctw::not_print}   = ${Char::Eeuctw::not_print};
${Char::Eeuctw::not_punct}   = ${Char::Eeuctw::not_punct};
${Char::Eeuctw::not_space}   = ${Char::Eeuctw::not_space};
${Char::Eeuctw::not_upper}   = ${Char::Eeuctw::not_upper};
${Char::Eeuctw::not_upper_i} = ${Char::Eeuctw::not_upper_i};
${Char::Eeuctw::not_word}    = ${Char::Eeuctw::not_word};
${Char::Eeuctw::not_xdigit}  = ${Char::Eeuctw::not_xdigit};
${Char::Eeuctw::eb}          = ${Char::Eeuctw::eb};
${Char::Eeuctw::eB}          = ${Char::Eeuctw::eB};
...
sub Char::Eeuctw::split(;$$$) {
    ...
    if (CORE::length($string) > 32766) {
        ...
        elsif (defined ${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17}) {
            $q_char = ${Char::Eeuctw::q_char_SADAHIRO_Tomoyuki_2002_01_17};
        }
    }
...
sub Char::Eeuctw::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Char::Eeuctw::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Char::Eeuctw::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Char::Eeuctw::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
sub Char::Eeuctw::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Char::Eeuctw::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Char::Eeuctw::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Char::Eeuctw::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
#
{
    ...
    sub Char::Eeuctw::matched() {
        $last_s_matched = 0;
    }

    # EUC-TW mark last s/// matched
    sub Char::Eeuctw::s_matched() {
        $last_s_matched = 1;
    }
    ...
    $Char::Eeuctw::matched = qr/(?{Char::Eeuctw::matched})/;
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Char::Eeuctw::uc($char[$i]);
                my $fc = Char::Eeuctw::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Char::Eeuctw::not_lower_i}',
                '[:^upper:]'  => '${Char::Eeuctw::not_upper_i}',
            ...
            $char[$i] = {
                ...
                '[:^alnum:]'  => '${Char::Eeuctw::not_alnum}',
                '[:^alpha:]'  => '${Char::Eeuctw::not_alpha}',
                '[:^ascii:]'  => '${Char::Eeuctw::not_ascii}',
                '[:^blank:]'  => '${Char::Eeuctw::not_blank}',
                '[:^cntrl:]'  => '${Char::Eeuctw::not_cntrl}',
                '[:^digit:]'  => '${Char::Eeuctw::not_digit}',
                '[:^graph:]'  => '${Char::Eeuctw::not_graph}',
                '[:^lower:]'  => '${Char::Eeuctw::not_lower}',
                '[:^print:]'  => '${Char::Eeuctw::not_print}',
                '[:^punct:]'  => '${Char::Eeuctw::not_punct}',
                '[:^space:]'  => '${Char::Eeuctw::not_space}',
                '[:^upper:]'  => '${Char::Eeuctw::not_upper}',
                '[:^word:]'   => '${Char::Eeuctw::not_word}',
                '[:^xdigit:]' => '${Char::Eeuctw::not_xdigit}',
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Char::Eeuctw::uc($char);
                        my $fc = Char::Eeuctw::fc($char);
                        if ($uc eq $fc) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Char::Eeuctw::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Char::Eeuctw::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Char::Eeuctw::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Char::Eeuctw::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
sub Char::EUCTW::getc(;*@) {
    ...
    croak 'Too many arguments for Char::EUCTW::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Char::Eeuctw::dot_s} \z/oxms) {
...
sub Char::EUCTW::index($$;$) {
    ...
    my $index;
    if (@_ == 3) {
        $index = Char::Eeuctw::index($_[0], $_[1], CORE::length(Char::EUCTW::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Char::Eeuctw::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Char::EUCTW::length(CORE::substr $_[0], 0, $index);
    }
}
...
sub Char::EUCTW::rindex($$;$) {
    ...
    my $rindex;
    if (@_ == 3) {
        $rindex = Char::Eeuctw::rindex($_[0], $_[1], CORE::length(Char::EUCTW::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Char::Eeuctw::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Char::EUCTW::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?: Char::EUCTW::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Eeuctw::matched};
$s_matched      = q{ Char::Eeuctw::s_matched();};
...
sub Char::EUCTW::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Char::EUCTW::escape_token();
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Eeuctw::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eeuctw::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Eeuctw::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eeuctw::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Eeuctw::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eeuctw::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Char::Eeuctw::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Eeuctw::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Eeuctw::glob';
    }
...
# subroutines of package Char::Eeuctw
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Char::EUCTW::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::EUCTW::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Char::EUCTW::escape'; }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::chop';       }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Char::EUCTW::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::EUCTW::index';       }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::index';      }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Char::EUCTW::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::EUCTW::rindex';      }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::rindex';     }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::lc';         }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::lcfirst';    }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::uc';         }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::ucfirst';    }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::fc';         }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeuctw::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Eeuctw::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Eeuctw::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Eeuctw::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Eeuctw::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Eeuctw::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Eeuctw::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Char::Eeuctw::PREMATCH()]}
            ...
            $e_string .= q{Char::Eeuctw::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Char::Eeuctw::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Char::Eeuctw::MATCH()};
            $slash = 'div';
        }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> @{[Char::Eeuctw::POSTMATCH()]}
            ...
            $e_string .= q{Char::Eeuctw::POSTMATCH()};
            $slash = 'div';
        }
...
# subroutines of package Char::Eeuctw
        ...
        elsif ($string =~ /\G \b Char::EUCTW::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::EUCTW::eval \b                                /oxgc) { $e_string .= 'eval Char::EUCTW::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Eeuctw::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::EUCTW::index \b                               /oxgc) { $e_string .= 'Char::EUCTW::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Eeuctw::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::EUCTW::rindex \b                              /oxgc) { $e_string .= 'Char::EUCTW::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Eeuctw::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeuctw::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Eeuctw::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Eeuctw::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Eeuctw::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Eeuctw::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Eeuctw::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Eeuctw::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Eeuctw::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Eeuctw::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Eeuctw::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Eeuctw::dot_s}';
        }
        else {
            return '${Char::Eeuctw::dot}';
        }
    }
    else {
        return Char::Eeuctw::classic_character_class($char);
    }
}
...
sub e_capture {
    ...
    return join '', '${Char::Eeuctw::capture(', $_[0], ')}';
    return join '', '${',                $_[0],  '}';
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Eeuctw::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Eeuctw::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Eeuctw::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Eeuctw::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Eeuctw::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Eeuctw::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Eeuctw::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Eeuctw::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Eeuctw::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Eeuctw::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Eeuctw::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Eeuctw::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Eeuctw::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Eeuctw::uc($char[$i]) ne Char::Eeuctw::fc($char[$i]))) {
            if (CORE::length(Char::Eeuctw::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Eeuctw::uc($char[$i])       . Char::Eeuctw::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Eeuctw::uc($char[$i]) . '|' . Char::Eeuctw::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeuctw::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Eeuctw::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeuctw::ignorecase(Char::Eeuctw::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eeuctw::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Eeuctw::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeuctw::ignorecase(Char::Eeuctw::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eeuctw::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Eeuctw::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeuctw::ignorecase(Char::Eeuctw::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eeuctw::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                                              2   3                                 4   5
                q<CORE::eval{local $Char::EUCTW::re_t=%s; local $Char::EUCTW::re_a=''; while($Char::EUCTW::re_t =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s$Char::EUCTW::re_t="$Char::EUCTW::re_a${1}$Char::EUCTW::re_r$'"; pos($Char::EUCTW::re_t)=length "$Char::EUCTW::re_a${1}$Char::EUCTW::re_r"; $Char::EUCTW::re_a=substr($Char::EUCTW::re_t,0,pos($Char::EUCTW::re_t)); } return $Char::EUCTW::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::EUCTW::re_r=CORE::eval $Char::EUCTW::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Char::EUCTW::re_t=%s; while($Char::EUCTW::re_t =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s$Char::EUCTW::re_t="$`$Char::EUCTW::re_r$'"; pos($Char::EUCTW::re_t)=length "$`$Char::EUCTW::re_r"; } return $Char::EUCTW::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Char::EUCTW::re_r=%s; %s"%s$Char::EUCTW::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $Char::EUCTW::re_n=0; local $Char::EUCTW::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="$Char::EUCTW::re_a${1}$Char::EUCTW::re_r$'"; pos(%s)=length "$Char::EUCTW::re_a${1}$Char::EUCTW::re_r"; $Char::EUCTW::re_a=substr(%s,0,pos(%s)); $Char::EUCTW::re_n++} return %s$Char::EUCTW::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Char::EUCTW::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="$`$Char::EUCTW::re_r$'"; pos(%s)=length "$`$Char::EUCTW::re_r"; $Char::EUCTW::re_n++} return %s$Char::EUCTW::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="%s$Char::EUCTW::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Char::EUCTW::re_r=%s; %s%s="%s$Char::EUCTW::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::EUCTW::re_r=CORE::eval $Char::EUCTW::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeuctw::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Eeuctw::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Eeuctw::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=head1 SYNOPSIS
  ...
  use Char::Eeuctw;
    ...
    Char::Eeuctw::lc_;
    Char::Eeuctw::lcfirst(...);
    Char::Eeuctw::lcfirst_;
    Char::Eeuctw::uc(...);
    Char::Eeuctw::uc_;
    Char::Eeuctw::ucfirst(...);
    Char::Eeuctw::ucfirst_;
    Char::Eeuctw::fc(...);
    Char::Eeuctw::fc_;
    Char::Eeuctw::ignorecase(...);
    Char::Eeuctw::capture(...);
    Char::Eeuctw::chr(...);
    Char::Eeuctw::chr_;
    Char::Eeuctw::glob(...);
  ...
  # "no Char::Eeuctw;" not supported

=head1 ABSTRACT

This module has run-time routines for use Char::EUCTW software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Char::EUCTW software and not
...
This Char::Eeuctw module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Char::Eeuctw::split(/pattern/,$string,$limit);
  @split = Char::Eeuctw::split(/pattern/,$string);
  @split = Char::Eeuctw::split(/pattern/);
  @split = Char::Eeuctw::split('',$string,$limit);
  @split = Char::Eeuctw::split('',$string);
  @split = Char::Eeuctw::split('');
  @split = Char::Eeuctw::split();
  @split = Char::Eeuctw::split;
  ...
  at all, Char::Eeuctw::split returns the original string as a single substring, If it
  ...
  @chars  = Char::Eeuctw::split(//,  $word);
  @fields = Char::Eeuctw::split(/:/, $line);
  @words  = Char::Eeuctw::split(" ", $paragraph);
  @lines  = Char::Eeuctw::split(/^/, $buffer);
  ...
  print join(":" => Char::Eeuctw::split(/ */, "hi there"));
  ...
  Char::Eeuctw::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Char::Eeuctw::split(/:/, $_, 3);
  ...
  splits on whitespace just as Char::Eeuctw::split with no arguments does. Thus,
  Char::Eeuctw::split(" ") can be used to emulate awk's default behavior. In contrast,
  Char::Eeuctw::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Char::Eeuctw::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Char::Eeuctw::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Char::Eeuctw::split()) {
          $count{$word}++;
      }
  }

  The inverse of Char::Eeuctw::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Char::Eeuctw::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Char::Eeuctw::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  print Char::Eeuctw::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Char::Eeuctw::chop(@list);
  $chop = Char::Eeuctw::chop();
  $chop = Char::Eeuctw::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Char::Eeuctw::chop subroutine is used primary to remove the newline
  ...
  since Char::Eeuctw::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Char::Eeuctw::chop a literal, only a variable. If you Char::Eeuctw::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Char::Eeuctw::chop(@lines);

  You can Char::Eeuctw::chop anything that is an lvalue, including an assignment:

  Char::Eeuctw::chop($cwd = `pwd`);
  Char::Eeuctw::chop($answer = <STDIN>);

  This is different from:

  $answer = Char::Eeuctw::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Char::Eeuctw::chop returns the character
  ...
  In the most general case, Char::Eeuctw::chop can be expressed using substr:

  $last_code = Char::Eeuctw::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Char::Eeuctw::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Char::Eeuctw::chop;
...
=item Index string
  ...
  $byte_pos = Char::Eeuctw::index($string,$substr,$byte_offset);
  $byte_pos = Char::Eeuctw::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Char::Eeuctw::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Char::Eeuctw::rindex($string,$substr,$byte_offset);
  $byte_pos = Char::Eeuctw::rindex($string,$substr);

  This subroutine works just like Char::Eeuctw::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Char::Eeuctw::index).
  ...
  while (($byte_pos = Char::EUCTW::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Char::Eeuctw::fc subroutine for case-insensitive comparisons via Char::EUCTW
  software.

=item Lower case first character of string

  $lcfirst = Char::Eeuctw::lcfirst($string);
  $lcfirst = Char::Eeuctw::lcfirst_;
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Char::Eeuctw::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Char::Eeuctw::ucfirst($string);
  $ucfirst = Char::Eeuctw::ucfirst_;
  ...
  $titlecase = Char::Eeuctw::ucfirst(substr($word,0,1)) . Char::Eeuctw::lc(substr($word,1));
  ...
  $do_not_use = Char::Eeuctw::ucfirst(Char::Eeuctw::lc($word));
...
=item Fold case string
  ...
  $fc = Char::Eeuctw::fc($string);
  $fc = Char::Eeuctw::fc_;

  New to Char::EUCTW software, this subroutine returns the full Unicode-like casefold of
  ...
  Char::Eeuctw::fc($a) eq Char::Eeuctw::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Char::EUCTW software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Char::Eeuctw::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Char::Eeuctw::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Char::Eeuctw::chr($code);
  $chr = Char::Eeuctw::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Char::Eeuctw::chr(65) is "A" in either
  ASCII or EUC-TW, not Unicode. For the reverse of Char::Eeuctw::chr, use Char::EUCTW::ord.

=item Filename expansion (globbing)

  @glob = Char::Eeuctw::glob($string);
  @glob = Char::Eeuctw::glob_;
  ...
  The Char::Eeuctw::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Char::Eeuctw::glob('"*e f*"');
  @spacies = Char::Eeuctw::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Char::Eeuctw::glob("'*${var}e f*'");
  @spacies = Char::Eeuctw::glob(qq("*${var}e f*"));
