...
my $function_ord;         # ord()   to ord()   or EUCTW::ord()
my $function_ord_;        # ord     to ord     or EUCTW::ord_
my $function_reverse;     # reverse to reverse or EUCTW::reverse
my $function_getc;        # getc    to getc    or EUCTW::getc
...
sub EUCTW::escape_script {
    ...
    if (/^ use Eeuctw(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ EUCTW \s* ([^\x8E\xA1-\xFE;]*) ; \s* \n? $//oxms) {
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'EUCTW::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'EUCTW::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'EUCTW::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'EUCTW::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $EUCTW::re_t=%s; local $EUCTW::re_a=''; while($EUCTW::re_t =~ %s){%s local $^W=0; local $EUCTW::re_r=%s; %s$EUCTW::re_t="$EUCTW::re_a${1}$EUCTW::re_r$'"; pos($EUCTW::re_t)=length "$EUCTW::re_a${1}$EUCTW::re_r"; $EUCTW::re_a=substr($EUCTW::re_t,0,pos($EUCTW::re_t)); } return $EUCTW::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$EUCTW::re_r=eval $EUCTW::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $EUCTW::re_t=%s; while($EUCTW::re_t =~ %s){%s local $^W=0; local $EUCTW::re_r=%s; %s$EUCTW::re_t="$`$EUCTW::re_r$'"; pos($EUCTW::re_t)=length "$`$EUCTW::re_r"; } return $EUCTW::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $EUCTW::re_r=%s; %s"%s$EUCTW::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $EUCTW::re_n=0; local $EUCTW::re_a=''; while(%s =~ %s){%s local $^W=0; local $EUCTW::re_r=%s; %s%s="$EUCTW::re_a${1}$EUCTW::re_r$'"; pos(%s)=length "$EUCTW::re_a${1}$EUCTW::re_r"; $EUCTW::re_a=substr(%s,0,pos(%s)); $EUCTW::re_n++} return %s$EUCTW::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $EUCTW::re_n=0; while(%s =~ %s){%s local $^W=0; local $EUCTW::re_r=%s; %s%s="$`$EUCTW::re_r$'"; pos(%s)=length "$`$EUCTW::re_r"; $EUCTW::re_n++} return %s$EUCTW::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $EUCTW::re_r=%s; %s%s="%s$EUCTW::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $EUCTW::re_r=%s; %s%s="%s$EUCTW::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$EUCTW::re_r=eval $EUCTW::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=head1 Multiple-Octet Anchoring of Regular Expression (EUCTW.pm provides)

EUCTW.pm applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Multiple-Octet Character Regular Expression (EUCTW.pm provides)

EUCTW.pm clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Eeuctw::ignorecase()' (EUCTW.pm provides)

EUCTW.pm applies calling 'Eeuctw::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   eval{ Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Eeuctw::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  eval{ Eeuctw::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Definitions in Eeuctw.pm.
  ...
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Eeuctw::anchor}         qr{\G(?:\x8E[\xA1-\xB0][\xA1-\xFE][\xA1-\xFE]|[\xA1-\xFE][\x00-\xFF]|[^\x8E\xA1-\xFE])*?}
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (EUCTW.pm provides)

EUCTW.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (EUCTW.pm and Eeuctw.pm provide)
...
=head1 Un-Escaping bytes::* Subroutines (EUCTW.pm provides)

EUCTW.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/EUCTW and change
'use utf8;' to 'use EUCTW;' in its. You need help yourself for now.
